...
from functions.bitcoin_info_retrieval import predict_price_for_specific_date, get_price_history_in_date_range
...
def get_function_calling_prompt(user_query):
    ...
    example = """{"name": "get_price_history_in_date_range", "arguments": {"date_start": "2024-01-10", "date_end": "2024-01-14"}}"""

    prompt = f"""<|im_start|>system
You are a helpful assistant with access to the following functions:

{serialize_function_to_json(predict_price_for_specific_date)}

{serialize_function_to_json(get_price_history_in_date_range)}

###INSTRUCTIONS:
- You need to choose one function to use and retrieve paramenters for this function from the user input.
- If the user query contains 'will' or 'tomorrow', it is very likely that you will need to use the predict_price_for_specific_date function.
...
EXAMPLE 2:
- User: Is the Bitcoin a safe investment or extremely volatile?
- AI Assistant: No Function needed.

EXAMPLE 3:
- User: When and what was the maximum bitcoin closing price from 2024-01-10 till 2024-01-14?
- AI Assistant:
<onefunctioncall>
...
def get_context_data(user_query: str, feature_view, model_deployment, model_llm=None, tokenizer=None, client=None) -> str:
    ...
    if functions:
        ...
        if isinstance(data, pd.DataFrame):
            if len(data) == 0:
                return f'Not information found about the Bitcoin prices on this date range'
            elif len(data) == 1:
                if "predicted_price" in data.columns:
                    return f'Bitcoin price prediction on the {functions[0]["arguments"]["date"]}:\n' + '\n'.join(
                        [f'Date: {row["date"]}; Predicted price: {row["predicted_price"]}' for _, row in data.iterrows()]
                    )
                else:
                    return f'Bitcoin price information on the {functions[0]["arguments"]["date_start"]}:\n' + '\n'.join(
                        [f'Date: {row["date"]}; Opening price: {row["open"]}; Closing price: {row["close"]}; Volume: {row["volume"]}; Highest price: {row["high"]}; Lowest price: {row["low"]}' for _, row in data.iterrows()]
                    ) 
            else:
                return f'Bitcoin prices information between {functions[0]["arguments"]["date_start"]} and {functions[0]["arguments"]["date_end"]}:\n' + '\n'.join(
                    [f'Date: {row["date"]}; Opening price: {row["open"]}; Closing price: {row["close"]}; Volume: {row["volume"]}; Highest price: {row["high"]}; Lowest price: {row["low"]}' for _, row in data.iterrows()]
                )
