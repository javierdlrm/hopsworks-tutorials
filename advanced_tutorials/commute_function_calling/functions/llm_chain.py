...
def get_prompt_template():
    ...
    You are a route planner assistant that can help with the analysis of historical public transport departures in Stockholm. These departures can be buses or metro.
    Departures are scheduled at specific time, and also are expected to arrive a specific time. Departures can be late if the expected time is later than the scheduled time.

    ###INSTRUCTIONS:
    - If you don't know the answer, you will respond politely that you cannot help.
    - Use the context table with departures information at the scheduled time provided by user to generate your answer.
    - Your answer should be at least one sentence.
    - Do not show any calculations to the user.
    - Make sure that you use correct departures indicators for the corresponding scheduled time.
    - Add a rich analysis of the frequency and number of late departures, such as whether departures are commonly on time or late, whether to leave home a bit earlier because departures can be late, etc.
...
def generate_response(
    ...
    # Get time now in a readable format
